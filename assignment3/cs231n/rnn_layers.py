...
def rnn_step_backward(dnext_h, cache):
    ...
    dout = dnext_h * (1 - np.square(next_h))
    dx = dout.dot(Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h_next = h0
    for i in range(T):
      h_next, cache_element = rnn_step_forward(x[:,i,:], h_next, Wx, Wh, b)
      h[:,i,:] = h_next
      cache.append(cache_element)
...
def rnn_backward(dh, cache):
    ...
    dWh = np.zeros((H,H))
    db = np.zeros(H)
    prev_h_step = np.zeros((N, H))
    for i in range(T - 1, -1, -1):
      cache_element = cache.pop()
      x_step, prev_h_step, Wx_step, Wh_step, b_step = rnn_step_backward(dh[:,i,:] + prev_h_step, cache_element)
      dx[:,i,:] = x_step
      dWx += Wx_step
      dWh += Wh_step
      db += b_step
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, Wx, prev_h, Wh, a_i, a_f, a_o, a_g, i, f, o, g, prev_c, next_c)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, Wx, prev_h, Wh, a_i, a_f, a_o, a_g, i, f, o, g, prev_c, next_c = cache
    ...
    dprev_c = f * dnext_c
    d_a_i = sigmoid(a_i) * (1 - sigmoid(a_i)) * di
    d_a_f = sigmoid(a_f) * (1 - sigmoid(a_f)) * df
    d_a_g = (1 - np.tanh(a_g)**2) * dg
    d_a_o = sigmoid(a_o) * (1 - sigmoid(a_o)) * do

    da = np.concatenate((d_a_i, d_a_f, d_a_o, d_a_g), axis=1)
    db = np.sum(da, axis=0)
    dWh = prev_h.T.dot(da) 
    dWx = x.T.dot(da)
    dprev_h = da.dot(Wh.T)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    hidden = h0
    cell = np.zeros(h0.shape)

    for i in range(T):
      next_h, next_c, cache_el = lstm_step_forward(x[:,i,:], hidden, cell, Wx, Wh, b)
      hidden = next_h
      h[:,i,:] = next_h
      cell = next_c
...
def lstm_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
      dx_step, dh_step, dcell_step, dWx_step, dWh_step, db_step = lstm_step_backward(dh[:,i,:] + dh_step, dcell, cache.pop())
      dx[:,i,:] = dx_step
      dcell = dcell_step
      dWx += dWx_step
      dWh += dWh_step
