...
def compute_saliency_maps(X, y, model):
    ...
    output = loss_f(out, y)
    output.backward()
    saliency = torch.abs(torch.max(X.grad, dim=1)[0])
...
def make_fooling_image(X, target_y, model):
    ...
    loss_f = torch.nn.CrossEntropyLoss()
    optim = torch.optim.SGD([X_fooling], learning_rate)
    ...
    for i in range(1000):
      ...
      if torch.tensor(target_y) == torch.argmax(score):
        break
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    print(loss)
    optim = torch.optim.SGD([img], learning_rate, weight_decay=l2_reg)
    loss.backward()
    optim.step()
