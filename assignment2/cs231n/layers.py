...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, mean, variance, x_hat, gamma, std, layernorm)
...
def batchnorm_backward(dout, cache):
    ...
    # cache(x, mean, variance, x_hat, gamma, std, layernorm)
    N = 1.0 * dout.shape[0]
    dgamma = np.sum(dout * cache[3], axis=cache[6])
    dbeta = np.sum(dout, axis=cache[6])
    dydx_hat = dout * cache[4]
    dx_hatdu = -1 / cache[5]
    dx_hatdv = -0.5 * (cache[2] ** (-1.5)) * (cache[0] - cache[1])
    dx_hatdx = 1 / cache[5]
    dvdx = 2 / N * (cache[0] - cache[1])
    dvdu = -2 / N * np.sum(cache[0] - cache[1], axis=0)
    dudx = 1 / N
    dx = dydx_hat * dx_hatdx + np.sum(dydx_hat * dx_hatdu, axis=0) * dudx + \
         np.sum(dydx_hat * dx_hatdv, axis=0) * (dvdx + dvdu * dudx)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    num_filters, channels, filter_height, filter_width = w.shape
    ...
    for n in range(num_examples):
      for h in range(out_height):
        for wi in range(out_width):
          ...
          out[n,:,h,wi] = np.sum(x_padded[n, :, h_start:h_end, w_start:w_end] * w, axis=(1,2,3)) + b
